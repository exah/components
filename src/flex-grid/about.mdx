...
- Default to `12` columns. Can be defined in `theme.flexGridSize`
...
- `column` — Number in range `1...theme.flexGridSize`
- `size` — Number in range `0...1`, result is percentage (`0.5` is `50%`)
- `offset` — Number in range `1...theme.flexGridSize` — move item by number of columns
...
`ThemeProvider` can be used to change defaults in runtime:

<Playground>
  <ThemeProvider theme={{ flexGridSize: 16 }}>
    <FlexGrid space={1} justifyContent='flex-end'>
      <FlexGrid.Item column={1}>
        <Box outline='1px solid' p={1}>
          1
        </Box>
